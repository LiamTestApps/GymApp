...
/** The full exercise write-up, without page chrome. Reused by the detail page
 *  and the swap preview popup so both always show identical content. */
export function ExerciseView({ id }: { id: string }) {
  ...
  // Reset when the exercise changes (popup gets reused for different ids).
  useEffect(() => { setFrame(0); setFailed(false); setPlaying(true) }, [id])

  if (!item) {
    return <p className="py-10 text-center text-[15px] text-muted">This exercise no longer exists.</p>
  }
...
export default function ExerciseDetail() {
  ...
  return (
    <>
      <TopBar back title={item?.name ?? 'Exercise'} />
